...
| 版本 bump | `npm run bump -- patch\|minor\|major`；加 `--tag` 则 commit + annotated tag（不 push） |
...
4. Release workflow 产出 draft → 干净 Win10/11 冒烟（WebView2、OCR 设置、识别）后 publish  

后续（未做）：Authenticode 签名、Tauri updater、macOS/Linux 矩阵。
