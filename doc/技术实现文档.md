...
| 模块 | 路径 | 职责 |
|------|------|------|
| AppShell | `shell/AppShell.vue` | home / tool / settings / toolConfig + 全局反馈 |
| ToolLauncher | `shell/ToolLauncher.vue` | 卡片 → `AppContextMenu`（打开 / 配置 / 关闭）；运行态 status 点 |
| ToolHeader | `shell/ToolHeader.vue` | 细顶栏：返回 + 工具名（无设置） |
...
| AppMessageHost | `components/common/AppMessageHost.vue` | 顶区堆叠 + TransitionGroup |
...
| 键 | 含义 | 备注 |
|----|------|------|
| `ocr.settings_version` | schema 版本，当前 `2` | 不符则 wipe 全部 `ocr.*` 再 bootstrap |
...
Commands：`get_setting` / `set_setting` / `get_appearance`；调试：`read_app_log` / `clear_app_log` / `get_logs_dir` / `open_logs_dir` / `open_devtools`（需 `app.debug_enabled=true`）；OCR：`recognize_image(path)` / `get_ocr_settings` / `save_ocr_settings` / `stage_image_file` / `stage_image_bytes` / `save_clipboard_image` / `rotate_image_orientation` / `clear_ocr_temp`（**recognize 不带密钥参数**；`set_setting` 拒写 `ocr.engine.*`）。
...
安全说明（须与产品文档一致）：v1 不加密；依赖 OS 用户目录 ACL；v1.1+ 评估 keyring。
...
| Key | 说明 |
|-----|------|
| `ocr.settings_version` | 当前 `2`；启动不符则 wipe `ocr.*` |
...
Baidu：OAuth → handwriting → `words_result`。
...
设置 IPC：`get_ocr_settings` / `save_ocr_settings`（描述符 + 值一次往返；FE 按 field 渲染；i18n 约定 `settings.ocrEngine_${id}` / `settings.ocrField_${name}`）。

Should 余项：百度 token 缓存。

### 4.3 Service 与 Command

| Command（示意名） | 行为 |
|-------------------|------|
| `recognize_image` | 参数：`path`；读文件 → Registry → 超时（默认 60s）→ `OcrResult`；**无** key 参数 |
| `get_ocr_settings` | 返回引擎列表 + 字段描述 + 当前 values |
| `save_ocr_settings` | 写 active/timeout + **仅当前引擎**字段；校验 key 归属 |
...
`OcrService`：组装路径、调用引擎、映射错误码、禁重入可由前端 busy + 后端可拒绝并发（二选一或双保险，技术实现优先前端禁点 + 单飞 Future）。

错误码示例：`ocr.not_configured`、`ocr.network`、`ocr.engine`、`ocr.timeout`、`ocr.bad_image`、`ocr.clipboard_empty`、`settings.*`。
...
└── OcrWorkspace.vue          // placement: left|right|bottom
    ...
    └── ResultInspector.vue   // 状态 chip + 重试 + 全文/词条；随 placement 停靠/折叠
tools/ocr/useOcr.ts
...
- 画布右键：打开、粘贴、显隐框、旋转、清除、条件重试。  
- 识别状态在 ResultInspector；失败可在结果窗重试。  
- 布局由 `ocr.inspector_placement` 决定（左/右/下）；全文与词条为 Tab 互斥（默认词条）。  
...
**已落地**：`AppButton`、`AppEmpty`、`AppContextMenu`、`AppMessage`、`AppMessageHost`、`ToolCard`、`AppPanel`。

**未实现（规划）**：`AppInput`、`AppSplit`、`AppConfirm`（需要时再补；禁止用死文件占位）。

OCR Inspector **不是**全局强制壳。
...
| 类型生成 | `mise run gen-types` |
| 检查 | `cargo check` + 前端 typecheck/lint |
| 打包 | `npm run tauri build`（NSIS/MSI） |

CI 建议：format/lint/check + `gen-types` 后 `git diff --exit-code src/api/generated`。
